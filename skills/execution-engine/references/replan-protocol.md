...
1. **Document the reason** in `.sdd-progress.md` under Learnings: what was discovered, why the plan is invalid, what the codebase already provides.
...
5. The agent updates `.sdd-state.json` (set phase back to `execution`, update `totalTasks` and `taskIndex`)
...
|---------|---------|--------|
| `replan.enabled` | `true` | `.sdd-preferences.yaml` |
| `replan.max_replans_per_session` | `2` | `.sdd-preferences.yaml` |

When max replans is reached, the loop halts with a message directing the user to review `.sdd-progress.md` and adjust tasks manually.
