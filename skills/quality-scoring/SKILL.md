...
description: |
  ...
  criteria coverage, and review resolution. Produces a numeric score that drives closure decisions.
...
**Important:** Score >= 80 makes the issue *eligible* for auto-close, but the closure rules matrix from `issue-lifecycle` still applies. A score of 90 on a human-assigned issue still requires human confirmation.
...
2. Apply threshold rules to determine action (auto-close, propose, block)
3. If auto-close eligible, check closure rules matrix (issue-lifecycle skill)
4. Execute the appropriate action with the score as evidence in the closing comment
